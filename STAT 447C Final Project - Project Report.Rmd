...
$$y_t \sim \text{Normal}(m_t + s_t + X_t + Z_t, \sigma^2_y)$$
...
$$m_t \sim \text{Normal}(m_{t-1}, \sigma^2_m)$$

where $m_{t-1}$ is the trend component at the previous time point, and $\sigma^2_m$
is the variance of the trend component.

*Seasonal Component:*
The seasonal component $s_t$ evolves over time as:
$$s_t \sim \text{Normal}\left(-\sum_{i=1}^{S-1} s_{t-i}, \sigma^2_{s}\right)$$

where $S$ is the seasonal period (in this case $S = 12$) for monthly, $s_{t-i}$ is
the seasonal component as the previous time point, and $\sigma^2_s$ is the variance
...
*White Noise:*
The white noise is given as:
$$Z_t \sim \text{Normal}(0, \sigma^2_Z)$$

where $\sigma^2_Z$ is the variance of the white noise.
\
...
*White Noise Variance:* $\sigma^2_Z \sim \text{Inverse-Gamma}(\alpha_Z, \beta_Z)$, 
where $\alpha_m = 2$ and $\beta_m = 0.004663646$.
\
...
noise variance, trend variance, seasonal variance, and white noise variance. Note, 
that we are assumed that the white noise variance is a fraction of the observation 
noise. 

The $\alpha$ (shape) values are set to $\alpha = 2$ to ensure that the prior is relatively
informative, thus allowing the data to play a significant role in the posterior distribution.
...
*White Noise Variance* $\sigma^2_Z$: An Inverse-Gamma distribution is used with 
$\alpha_Z$ and $\beta_Z$ values set based on random variability in the data, thus
accounting for the unexplained noise in the data.
\
...
**Section 2: Creating a Time Series Object for the Training Set**
...
*SAP HANA Predictive Analysis Library (PAL)*, help.sap.com/docs/

SAP_HANA_PLATFORM/
2cfbc5cf2bc14f028cfbe2a2bba60a50/
b9972576368640da9831d73a9d749c3b.html. 
Accessed 8 Apr. 2024. 
...
white_noise_variance_estimate <- obs_noise_variance_estimate * 0.1
...
beta_white_noise <- white_noise_variance_estimate * (alpha - 1)
...
cat("Inverse-Gamma beta for White Noise:", beta_white_noise, "\n")
...
Please see below the implementation of the BSTS model on US unemployment rate using
Stan.
...
parameters {
  ...
  vector[T] Zt;                     // White noise component
  real<lower=0> sigma_y;            // Standard deviation of observation noise
  real<lower=0> sigma_m;            // Standard deviation of trend component
  real<lower=0> sigma_s;            // Standard deviation of seasonal component
  real<lower=0> sigma_Z;            // Standard deviation of white noise component
...
model {
  ...
  sigma_Z ~ inv_gamma(2, 0.004663646);
  phi ~ normal(0, 1);
  ...
  // Seasonal component
  // Assuming S = 12 for monthly data
  for (t in 13:T) {
  ...
  for (t in 2:T) {
    y[t] ~ normal(mt[t] + st[t] + phi * y[t-1] + Zt[t], sigma_y);
  }
}
"
